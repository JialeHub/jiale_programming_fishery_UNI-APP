<template>
	<view style="display: flex;flex-direction: column;">
		...
		<u-form ref="uForm" :model="form" :error-type="['toast']" style='border-top: rgba(240,240,240,1) 2rpx solid;flex: 1 1 auto;background-color: #FFF;padding:0 16rpx;'>
			<u-form-item label="渔船船号" prop='fishingBoats' label-width="5rem" >
				<u-input v-model="form.fishingBoats" type='text' placeholder="" :trim="true" />
			</u-form-item>
		</u-form>
		...
		methods: {
			save() {
				this.$refs.uForm.validate(async valid => {
					if (!valid) return
					let data = {fishingBoats:this.form.fishingBoats};
					uni.showLoading();
