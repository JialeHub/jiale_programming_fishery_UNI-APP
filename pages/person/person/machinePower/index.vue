<template>
	<view style="display: flex;flex-direction: column;">
		...
		<u-form ref="uForm" :model="form" :error-type="['toast']" style='border-top: rgba(240,240,240,1) 2rpx solid;flex: 1 1 auto;background-color: #FFF;padding:0 16rpx;'>
			<u-form-item label="主机功率(千瓦)" prop='machinePower' label-width="6.5rem">
				<u-input v-model="form.machinePower" type='number' placeholder="" :trim="true" />
			</u-form-item>
		</u-form>
		...
		methods: {
			save() {
				this.$refs.uForm.validate(async valid => {
					if (!valid) return
					let data = {machinePower:this.form.machinePower};
					uni.showLoading();
