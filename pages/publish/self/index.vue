<template>
	<view class="content" style="padding: 20rpx;">
		<!-- 登录 -->
		<u-form :model="form" ref="uForm" style="" :error-type="['toast']" label-width='200'>
			<view class="wrap">
			...
			<u-form-item label="作业类型" prop="operatingType" required>
				<u-input v-model="selectorType[form['operatingType']]" :placeholder="'请前往个人中心选择作业类型'" confirm-type='next' disabled />
			</u-form-item>
			<u-form-item label="船号" prop="fishingBoats" required>
				<u-input v-model="form.fishingBoats" confirm-type='next'  :placeholder="'请前往个人中心设置船号'" disabled />
			</u-form-item>
			<u-form-item label="主机功率(千瓦)" prop="machinePower" required>
				<u-input v-model="form.machinePower" confirm-type='next' :placeholder="'请前往个人中心设置主机功率'" disabled />
			</u-form-item>
...
<script>
	export default {
		...
		created() {
			this.form.fishingBoats=this.userInfo['fishingBoats']
			this.form.machinePower=this.userInfo['machinePower']
			this.form.operatingType=this.userInfo['operatingType']
			// 监听从裁剪页发布的事件，获得裁剪结果
